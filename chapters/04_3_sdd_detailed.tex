...
Các phương thức (Methods) của từng lớp trong hệ thống mô tả hành vi động và các xử lý logic cụ thể của các thực thể nghiệp vụ:

\begin{table}[H]
\centering
\renewcommand{\arraystretch}{1.3}
\begin{tabularx}{\textwidth}{|p{3cm}|p{5cm}|>{\raggedright\arraybackslash}X|}
\hline
\rowcolor{tableheader}
\textbf{Tên lớp} & \textbf{Tên phương thức} & \textbf{Mô tả chức năng} \\ \hline
Role & getRoleName(), getDescription() & Lấy thông tin chi tiết của vai trò. \\ \hline
User & login(), logout(), updateProfile(), changePassword(), deactivateAccount() & Xác thực người dùng, cập nhật thông tin và quản lý tài khoản. \\ \hline
Series & createSeries(), updateSeries(), deleteSeries(), publishSeries(), getChapters() & Quản lý vòng đời bộ truyện, xuất bản và truy vấn danh sách chương. \\ \hline
Chapter & createChapter(), updateChapter(), publishChapter(), getPages() & Quản lý thông tin chương truyện, phê duyệt trang và phát hành. \\ \hline
Page & addPage(), updatePage(), deletePage() & Quản lý các trang ảnh trong chương truyện. \\ \hline
Task & createTask(), assignTask(), updateTaskStatus(), cancelTask() & Giao việc cho trợ lý (Assistant), cập nhật tiến độ công việc. \\ \hline
Submission & submitWork(), updateSubmission(), deleteSubmission(), getReviews() & Trợ lý nộp sản phẩm, cập nhật file nộp và xem phản hồi. \\ \hline
Review & createReview(), updateReview(), getReviewDetails() & Biên tập viên/Tác giả đánh giá bài nộp, chấm điểm và viết nhận xét. \\ \hline
SeriesRanking & updateRank(), getRankingHistory() & Tính toán điểm số và cập nhật xếp hạng của bộ truyện theo kỳ. \\ \hline
Notification & createNotification(), markAsRead(), deleteNotification() & Gửi thông báo đến người nhận, đánh dấu đã đọc hoặc xóa. \\ \hline
\end{tabularx}
\caption{Mô tả chi tiết phương thức của các lớp trong hệ thống}
\label{tab:class_methods}
\end{table}
