...
\textbf{Mô tả} & Cập nhật trạng thái của trang trong quy trình vẽ (drafting, drawing, reviewing\_draft, reviewing\_final, approved, published). \\ \hline
...
\begin{minipage}[t]{\linewidth}
    \begin{enumerate}[leftmargin=*]
        ...
        \item Kéo thả Task (hoặc bấm chọn) để thay đổi trạng thái sang \texttt{'in\_progress'} (Assistant tự cập nhật) hoặc nộp bản vẽ để chuyển sang \texttt{'submitted'} (sau đó Mangaka sẽ phê duyệt để thành \texttt{'completed'} hoặc từ chối để thành \texttt{'rejected'}).
        \item Hệ thống ghi nhận trạng thái, thời gian cập nhật.
        \item Thông báo tự động gửi đến Mangaka nếu trạng thái là 'submitted'.
    \end{enumerate}
\end{minipage} \\ \hline
\textbf{Luồng rẽ nhánh/Ngoại lệ (Alternate/Exception)} & 
\begin{minipage}[t]{\linewidth}
...
\begin{minipage}[t]{\linewidth}
    \begin{enumerate}[leftmargin=*]
        ...
        \item Nhấn 'Gửi'. Hệ thống lưu file lên Cloud Storage, chuyển trạng thái Task sang 'submitted' và gửi Notification cho Mangaka.
    \end{enumerate}
\end{minipage} \\ \hline
\textbf{Luồng rẽ nhánh/Ngoại lệ (Alternate/Exception)} & 
\begin{minipage}[t]{\linewidth}
...
\begin{minipage}[t]{\linewidth}
    ...
    \end{itemize}
\end{minipage} \\ \hline
\textbf{Hậu điều kiện (Post-conditions)} & Các Annotation (được lưu trữ trong bảng \texttt{editor\_annotations} liên kết với trang truyện) và kết quả Review được lưu trữ vĩnh viễn, trạng thái Task thay đổi tương ứng. \\ \hline
\end{tabularx}
...
\textbf{Mô tả} & Đánh giá, bỏ phiếu và phê duyệt một Series mới (Pilot/One-shot) được Mangaka đề xuất. \\ \hline
\textbf{Tiền điều kiện (Pre-conditions)} & Tài khoản thuộc nhóm Editorial Board, tồn tại Series ở trạng thái 'Pending Approval'. \\ \hline
\textbf{Luồng sự kiện chính (Basic Flow)} & 
\begin{minipage}[t]{\linewidth}
    \begin{enumerate}[leftmargin=*]
        \item Hội đồng xem xét tài liệu Pitching, Kịch bản One-shot của Series.
        \item Tổ chức tính năng họp bàn hoặc bỏ phiếu nội bộ trên hệ thống.
        \item Tổng biên tập (đại diện Hội đồng) chọn 'Approve' (Duyệt) hoặc 'Reject' (Từ chối).
        \item Hệ thống cập nhật trạng thái Series (Thành 'Ongoing' nếu duyệt) và gửi thư phản hồi tự động.
    \end{enumerate}
\end{minipage} \\ \hline
\textbf{Luồng rẽ nhánh/Ngoại lệ (Alternate/Exception)} & 
\begin{minipage}[t]{\linewidth}
    \begin{itemize}[leftmargin=*]
        \item \textbf{A1}: Ý tưởng tốt nhưng cần sửa lại kịch bản. Hội đồng chọn 'Needs Revision', điền yêu cầu sửa và trả lại cho tác giả.
    \end{itemize}
\end{minipage} \\ \hline
\textbf{Hậu điều kiện (Post-conditions)} & Series có trạng thái pháp lý và hoạt động chính thức trên nền tảng. \\ \hline
\end{tabularx}
\caption{Đặc tả UC-26: Xét duyệt Series mới}
\label{tab:uc26}
\end{table}

\subsubsection{UC-27: Xuất bản Chapter}\label{subsubsec:uc27_publish_chapter}
\begin{table}[H]
\centering
\renewcommand{\arraystretch}{1.3}
\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash\bfseries}p{4.5cm}|>{\raggedright\arraybackslash}X|}
\hline
\rowcolor{tableheader}
\multicolumn{2}{|c|}{\textbf{Đặc tả Use Case: Xuất bản Chapter}} \\ \hline
\textbf{Mã Use Case} & UC-27 \\ \hline
\textbf{Tên Use Case} & Xuất bản Chapter \\ \hline
\textbf{Actor chính} & Editorial Board \\ \hline
\textbf{Mô tả} & Phát hành Chapter ra công chúng, website hoặc nền tảng đọc trực tuyến. \\ \hline
\textbf{Tiền điều kiện (Pre-conditions)} & Chapter đã hoàn thiện 100\%, hoàn tất Review và được Tantou Editor thông qua (Approved). \\ \hline
\textbf{Luồng sự kiện chính (Basic Flow)} & 
\begin{minipage}[t]{\linewidth}
    \begin{enumerate}[leftmargin=*]
        \item Hội đồng kiểm tra chất lượng file lần cuối.
        \item Chọn Chapter, thiết lập ngày giờ phát hành (Timezone mặc định).
        \item Nhấn 'Publish'.
        \item Hệ thống đổi trạng thái Chapter thành 'Published', đóng dấu Watermark chống vi phạm bản quyền và phân phối tới Database đọc truyện.
    \end{enumerate}
\end{minipage} \\ \hline
\textbf{Luồng rẽ nhánh/Ngoại lệ (Alternate/Exception)} & 
\begin{minipage}[t]{\linewidth}
    \begin{itemize}[leftmargin=*]
        \item \textbf{A1}: Thiết lập lịch phát hành tự động (Scheduled Publish). Hệ thống sẽ chuyển trạng thái 'Scheduled' và chờ Trigger tự động đổi thành 'Published' khi đến giờ G.
    \end{itemize}
\end{minipage} \\ \hline
\textbf{Hậu điều kiện (Post-conditions)} & Chapter được phát hành chính thức, người dùng không thể sửa đổi nội dung cốt lõi nữa. \\ \hline
\end{tabularx}
\caption{Đặc tả UC-27: Xuất bản Chapter}
\label{tab:uc27}
\end{table}

\subsubsection{UC-28: Quyết định ngừng phát hành}\label{subsubsec:uc28_cease_publish}
\begin{table}[H]
\centering
\renewcommand{\arraystretch}{1.3}
\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash\bfseries}p{4.5cm}|>{\raggedright\arraybackslash}X|}
\hline
\rowcolor{tableheader}
\multicolumn{2}{|c|}{\textbf{Đặc tả Use Case: Quyết định ngừng phát hành}} \\ \hline
\textbf{Mã Use Case} & UC-28 \\ \hline
\textbf{Tên Use Case} & Quyết định ngừng phát hành \\ \hline
\textbf{Actor chính} & Editorial Board \\ \hline
\textbf{Mô tả} & Đình chỉ (hiatus) hoặc kết thúc (axe) một Series do thành tích xếp hạng kém hoặc lý do cá nhân của tác giả. \\ \hline
\textbf{Tiền điều kiện (Pre-conditions)} & Series đang ở trạng thái phát hành 'Ongoing'. \\ \hline
\textbf{Luồng sự kiện chính (Basic Flow)} & 
\begin{minipage}[t]{\linewidth}
    \begin{enumerate}[leftmargin=*]
        \item Hội đồng chọn Series cần xử lý.
        \item Chọn 'Change Status' $\rightarrow$ 'Cancelled' (Hủy bỏ) hoặc 'Hiatus' (Tạm ngưng).
        \item Nhập lý do (bắt buộc để lưu hồ sơ).
        \item Hệ thống lưu trạng thái, tự động đóng băng toàn bộ các Task/Chapter đang mở thuộc Series và thông báo cho toàn đội ngũ sản xuất.
    \end{enumerate}
\end{minipage} \\ \hline
\textbf{Luồng rẽ nhánh/Ngoại lệ (Alternate/Exception)} & 
\begin{minipage}[t]{\linewidth}
    \begin{itemize}[leftmargin=*]
        \item \textbf{A1}: Tạm ngưng có thời hạn vì lý do sức khỏe. Người dùng thiết lập thêm 'Ngày dự kiến mở lại', hệ thống sẽ gửi nhắc nhở khi sắp đến hạn.
    \end{itemize}
\end{minipage} \\ \hline
\textbf{Hậu điều kiện (Post-conditions)} & Series bị dừng vô thời hạn hoặc có thời hạn, quy trình sản xuất tạm khóa. \\ \hline
\end{tabularx}
\caption{Đặc tả UC-28: Quyết định ngừng phát hành}
\label{tab:uc28}
\end{table}
...
\textbf{Actor chính} & Admin, Editorial Board \\ \hline
\textbf{Mô tả} & Nhập dữ liệu phiếu bầu (vote) của độc giả từ tạp chí giấy hoặc API hệ thống ngoài. \\ \hline
\textbf{Tiền điều kiện (Pre-conditions)} & Tài khoản có quyền Admin/Editor. Đã có dữ liệu thô từ bộ phận khảo sát độc giả. \\ \hline
\textbf{Luồng sự kiện chính (Basic Flow)} & 
\begin{minipage}[t]{\linewidth}
    \begin{enumerate}[leftmargin=*]
        \item Người dùng vào module 'Quản lý xếp hạng'.
        \item Chọn Kỳ phát hành tạp chí (Issue Number).
        \item Nhập thủ công số phiếu bầu hoặc điểm số cho từng Series đang phát hành.
        \item Nhấn 'Tính toán \& Lưu'.
        \item Hệ thống cập nhật bảng xếp hạng cho kỳ đó vào Database.
    \end{enumerate}
\end{minipage} \\ \hline
\textbf{Luồng rẽ nhánh/Ngoại lệ (Alternate/Exception)} & 
\begin{minipage}[t]{\linewidth}
    \begin{itemize}[leftmargin=*]
        \item \textbf{A1}: Import hàng loạt từ file Excel (.xlsx). Hệ thống tự động đọc file, ánh xạ cột dữ liệu và tính toán thứ hạng giảm dần lỗi nhập tay.
    \end{itemize}
\end{minipage} \\ \hline
\textbf{Hậu điều kiện (Post-conditions)} & Bảng xếp hạng của kỳ phát hành được cập nhật và đóng băng chỉnh sửa. Đồng thời, hệ thống tự động gửi thông báo cảnh báo loại \texttt{series\_warning} cho Mangaka của truyện nếu điểm quy chuẩn dưới 50 điểm đồng thời thứ hạng của truyện từ hạng 5 trở xuống. \\ \hline
\end{tabularx}
