...
\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash\bfseries}p{4.0cm}|>{\raggedright\arraybackslash}X|}
\hline
...
\hline
\rowcolor{tableheader}
\multicolumn{2}{|c|}{\textbf{Đặc tả yêu cầu Web: Cập nhật phiếu bầu và xếp hạng}} \\ \hline
\textbf{HTTP Method / Route} & \texttt{POST /index.php?}\allowbreak\texttt{controller=seriesranking\&}\allowbreak\texttt{action=update} \\ \hline
\textbf{Quyền truy cập (Role)} & Editorial Board (Ban biên tập) \\ \hline
\textbf{Mô tả nghiệp vụ} & Nhập số lượng phiếu bình chọn của độc giả thu được cho Series Manga trong kỳ xuất bản hiện tại để hệ thống tự động cập nhật lại thứ hạng. \\ \hline
\textbf{Tham số đầu vào (Request Body)} & 
\begin{minipage}[t]{\linewidth}
    \begin{itemize}[leftmargin=*]
        \item \texttt{series\_id} (Số nguyên, bắt buộc): ID bộ truyện cần cập nhật.
        \item \texttt{period\_start\_date} (Định dạng Ngày, bắt buộc): Ngày bắt đầu kỳ bình chọn hiện tại.
        \item \texttt{votes} (Số nguyên, bắt buộc): Số lượng phiếu bầu độc giả hợp lệ nhận được trong kỳ.
    \end{itemize}
\end{minipage} \\ \hline
\textbf{Logic xử lý trên Server} & 
\begin{minipage}[t]{\linewidth}
    \begin{enumerate}[leftmargin=*]
        \item Kiểm tra quyền hạn Ban biên tập (Editorial Board) của người dùng.
        \item INSERT hoặc UPDATE bản ghi vào bảng \texttt{series\_rankings} với dữ liệu phiếu bầu và ngày kỳ.
        \item Tự động chạy thuật toán xếp hạng: Tính tổng số phiếu bầu và sắp xếp giảm dần tất cả các bộ truyện để ghi đè cột thứ tự xếp hạng (\texttt{rank\_position}) trong kỳ đó.
        \item Tạo và gửi thông báo cập nhật bảng xếp hạng định kỳ gửi cho toàn bộ người dùng hệ thống.
    \end{enumerate}
\end{minipage} \\ \hline
\textbf{Kết quả phản hồi (Response)} & 
\begin{minipage}[t]{\linewidth}
    \begin{itemize}[leftmargin=*]
        \item \textbf{Thành công (HTTP 302):} Quay lại trang quản trị Bảng xếp hạng, thông báo "Cập nhật dữ liệu xếp hạng thành công".
        \item \textbf{Thất bại (HTTP 302):} Quay lại trang nhập dữ liệu kèm báo cáo lỗi (ví dụ: "Số lượng phiếu bình chọn không được nhỏ hơn 0.").
    \end{itemize}
\end{minipage} \\ \hline
\end{tabularx}
